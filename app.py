...
def alpha_daily_adjusted_csv(ticker: str, api_key: str, outputsize: str = "full") -> pd.DataFrame:
    """
    TIME_SERIES_DAILY_ADJUSTED en CSV (OHLC, adjusted close, volume, dividend, split).
    outputsize: 'compact' (≈100 últimas) o 'full' (toda la historia).
    """
    params = {
    ...
    r = requests.get(ALPHA_BASE, params=params, timeout=25)
    r.raise_for_status()
    # Alpha devuelve CSV; si supera límites, devuelve JSON con 'Note' -> detectarlo
    if r.text.lstrip().startswith("{"):
        # mensaje de límite o error
        msg = r.json().get("Note") or r.json().get("Error Message") or "Respuesta JSON no esperada"
...
def alpha_fetch_window(ticker: str, api_key: str, start: date, end: date, outputsize="full") -> pd.DataFrame:
    """
    Trae diario ajustado y recorta por fechas.
    Incluye backoff simple si aparece mensaje de límite (5 req/min en plan free).
    """
    # hasta 2 reintentos suaves
    delays = [0, 15]
    last_err = None
    for d in delays:
        if d: time.sleep(d)
        try:
            df = alpha_daily_adjusted_csv(ticker, api_key, outputsize=outputsize)
...
if pagina == "📊 Exploración de Datos":
    ...
    with c3:
        outputsize = st.selectbox("Tamaño", ["compact (≈100 días)","full (todo)"], index=1)
    ...
    for tk in ["AMZN", "ORCL"]:
        ...
        try:
            ...
            price_col = "Adj Close" if "Adj Close" in df_eq.columns else "Close"
    ...
    st.info("Tip: para portafolios usa **Adj Close** (acciones) y **Close/VWAP** (BTC) para retornos. "
            "Alpha Vantage free permite ~5 requests/min; con AMZN y ORCL estás dentro del límite.")
